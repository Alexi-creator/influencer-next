/* eslint-disable max-len */

export const LoadingIcon = ({ ...props }) => (
  <svg
    viewBox="0 0 24 24"
    fill="none"
    stroke="currentColor"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <g fill="none" fillRule="evenodd" clipRule="evenodd">
      <path
        d="M12 2.5C6.75329 2.5 2.5 6.75329 2.5 12C2.5 17.2467 6.75329 21.5 12 21.5C17.2467 21.5 21.5 17.2467 21.5 12C21.5 6.75329 17.2467 2.5 12 2.5ZM1 12C1 5.92487 5.92487 1 12 1C18.0751 1 23 5.92487 23 12C23 18.0751 18.0751 23 12 23C5.92487 23 1 18.0751 1 12Z"
        opacity=".2"
      />
      <path d="M11.25 1.75C11.25 1.33579 11.5858 1 12 1C18.0751 1 23 5.92487 23 12C23 12.4142 22.6642 12.75 22.25 12.75C21.8358 12.75 21.5 12.4142 21.5 12C21.5 6.75329 17.2467 2.5 12 2.5C11.5858 2.5 11.25 2.16421 11.25 1.75Z" />
    </g>
  </svg>
)
